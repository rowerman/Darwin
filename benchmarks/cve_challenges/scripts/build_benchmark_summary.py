...
REPO_ROOT = CVE_ROOT.parent.parent
...
EXCLUDED_CHAINS: frozenset[str] = frozenset(
    {
        "esc3-to-dcsync",
        "redis-to-golden",
        "web-to-da",
...
EXCLUDED_SCENARIO_REASONS: dict[str, str] = {
    "lnx-01": "需 VT-x 嵌套虚拟化或 QEMU 内核镜像",
    "lnx-02": "需 VT-x 嵌套虚拟化或 QEMU 内核镜像",
    "lnx-03": "需 VT-x 嵌套虚拟化或 QEMU 内核镜像",
    "lnx-04": "需 VT-x 嵌套虚拟化或 QEMU 内核镜像",
    "ad-03": "Windows LDAP 驱动漏洞",
    "ad-04": "Windows AD CS（ESC3）",
    "ad-06": "Windows AD CS（ESC8）",
    "ad-07": "Windows Kerberos/SMB 反射",
    "ad-08": "Windows Kerberos/SMB 反射",
    "ad-11": "Windows Server 2025 BadSuccessor",
    "ad-12": "Windows 性能计数器漏洞",
}

EXCLUDED_CHAIN_REASONS: dict[str, str] = {
    "esc3-to-dcsync": "完全阻塞：Windows AD CS / AD-04",
    "redis-to-golden": "部分阻塞：LNX-04 + AD-06 ESC8",
    "web-to-da": "关键步骤 LNX-01（Vagrant/QEMU）",
    "mysql-to-cluster": "关键步骤 LNX-01",
    "wordpress-to-k8s": "关键步骤 LNX-02",
}
...
def chain_doc_paths() -> list[Path]:
    ...
    for path in chain_docs:
        name = path.stem.replace("-exploitation", "")
        if name in EXCLUDED_CHAINS:
            continue
        result.append(path)
    return result
...
def load_scenarios_registry() -> list[dict]:
    ...
    for key, entry in sorted(data.get("scenarios", {}).items(), key=lambda x: x[0]):
        if key in EXCLUDED_SCENARIOS:
            continue
        if entry.get("platform") == "windows-only":
            continue
        rows.append(
            {
                "key": key,
...
def build_exclusion_section() -> str:
    lines = ["## 已排除范围（不可实例化测试）", ""]
    lines.append("以下场景/攻击链**未纳入**本文正文，详见 "
                 "[benchmarks/cve_challenges/README.md](benchmarks/cve_challenges/README.md) 阻塞明细。")
    lines.append("")
    lines.append("### 排除的单点场景")
    lines.append("")
    lines.append("| 场景 ID | 原因 |")
    lines.append("|---------|------|")
    for sid in sorted(EXCLUDED_SCENARIOS):
        lines.append(f"| {sid.upper()} | {EXCLUDED_SCENARIO_REASONS.get(sid, '阻塞')} |")
    lines.append("")
    lines.append("### 排除的攻击链")
    lines.append("")
    lines.append("| 链目录 | 原因 |")
    ...
    for chain in sorted(EXCLUDED_CHAINS):
        lines.append(f"| `{chain}` | {EXCLUDED_CHAIN_REASONS.get(chain, '阻塞')} |")
    lines.append("")
    return "\n".join(lines)
...
def build_toc(included_sources: list[tuple[str, str]], n_chains: int) -> str:
    lines = ["## 目录", ""]
    lines.append("- [已排除范围](#已排除范围不可实例化测试)")
...
def main() -> None:
    ...
    parts.append(
        "> **全量设计**（含阻塞场景）：[docs/cve-benchmark-comprehensive-guide.md](docs/cve-benchmark-comprehensive-guide.md)"
    )
    parts.append("")
    parts.append(build_exclusion_section())
    ...
    print(f"  Excluded chains: {len(EXCLUDED_CHAINS)}")
