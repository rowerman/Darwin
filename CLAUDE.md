...
- **DarwinRAG static knowledge**: 8125 curated entries across 4 domain collections (web, windows_ad, cloud, network), continuously expanded via `tools/ingest_knowledge.py`. SentenceTransformer `all-MiniLM-L6-v2` (384-dim) + Faiss IndexFlatIP with TfidfVectorizer fallback.
...
The `run()` method (`orchestrator.py:224`) follows this linear phase pipeline for Solo mode. For Coordinated/Distributed modes, it dispatches to `_run_multi_agent_cycle()` (`orchestrator.py:8802`) based on the B dimension threshold from `dynamic_scaling.py`.

Key `Orchestrator` method locations (~9565-line file; line numbers approximate):
- `run()` (~line 224) — entry point, main loop with mode dispatch, termination conditions
- `_detect_chain_topology()` (~line 2170) — auto-detects multi-flag attack chains from service topology (chain_mode: auto)
- `_unified_llm_loop()` (~line 2475) — Solo mode LLM-driven ReAct loop
- `_execute_privesc()` (~line 3335) — auto-detects and exploits Linux privesc vectors (SUID, writable /etc/passwd, Docker socket, capabilities, cron, LD_PRELOAD) from `linux_priv_check` output
- `_try_db_default_credentials()` (~line 3475) — auto-trials known default credentials against discovered DB services (MySQL/PostgreSQL/Redis/MSSQL/Oracle/MongoDB)
- `_analyze_phase()` (~line 4072) — vulnerability hypothesis generation; auto re-triggers when new services/endpoints discovered (>2 new), capped at 2 re-analyses via `_analyze_service_snapshot`/`_reanalyze_count`
- `_sanitize_plan_tools()` (~line 5499) — filters/rewrites plan tasks (blacklist, fallback, credential placeholder resolution, file:// detection)
- `_generate_exploitation_plan()` (~line 6036) — LLM exploitation strategy
- `_get_next_ready_task()` inline at ~line 6669 — task prioritization via `_EXPLOIT_PRIORITY` (container escape, K8s, cloud, post-exploit tools) and `_LOW_PRIORITY` (brute force) sets
- `_extract_recent_artifacts()` (~line 6750) — extracts recently discovered credentials/sessions/endpoints for cross-step context injection
- `_build_defense_evasion_context()` (~line 6832) — injects WAF/Honey/Trap/Cloak evasion guidance (encoding mutations, LOTL command substitution, honeypot vigilance) into plan generation prompts
- `_analyze_and_fix_task()` (~line 7035) — LLM-based task failure classification and retry (up to 2 attempts); partial successes (auth succeeds but sub-command fails) extract partial value
- `_review_and_update_plan()` (~line 7320) — dedup merged plans (tool+endpoint matching + >75% word overlap), remap stale task ID references in dependencies
- `_exploit_phase()` (~line 7983) — tool execution and verification
- `_build_truncation_context()` (~line 8690) — builds structured DKG summary (flags/creds/sessions/services/vulns) for context compression fallback
- `_run_multi_agent_cycle()` (~line 8802) — Coordinated/Distributed dispatch

### Module roles

| Module | Role |
|--------|------|
| `orchestrator.py` | Main loop with 7 termination conditions. Solo mode: `_sanitize_plan_tools()` filters tasks, `_analyze_and_fix_task()` retries fixable failures, `_review_and_update_plan()` deduplicates merged plans and remaps stale dependency references, deterministic tools execute directly (non-LLM). `_run_multi_agent_cycle()` handles Coordinated/Distributed modes. Auto-detects multi-flag attack chains (`_detect_chain_topology()`). Prioritizes high-value exploit tasks over speculative recon (`_EXPLOIT_PRIORITY`/`_LOW_PRIORITY`). Tracks `_absent_services` (unreachable), `_BLACKLISTED_TOOLS` (broken), `_TOOL_FALLBACK` (alternatives). Auto-exploits Linux privesc vectors (`_execute_privesc()`), trials DB default credentials (`_try_db_default_credentials()`), injects intermediate artifacts for multi-step exploit continuity (`_extract_recent_artifacts()`), and injects defense evasion guidance into plans (`_build_defense_evasion_context()`). |
...
| `cloud_topology.py` | CTAGE (Cloud Topology & Attack Graph Engine, 809 lines): auto-discovers K8s cluster topology (nodes/pods/namespaces/SAs/RBAC bindings), analyzes Pod security contexts (privileged/capabilities/hostPID/mounts) and matches escape vectors, enumerates IAM roles and cross-account trust relationships. Outputs structured DKG nodes+edges. |
| `cloud_attack_path.py` | AttackPathReasoner (486 lines): four BFS path discoverers — IAM privilege escalation (role_can_assume chains), container escape (security config→tool matching), lateral movement (cross-namespace RBAC), cross-account (TrustRelationship traversal). Ranked by difficulty and confidence, outputs LLM-ready prompt injection. |
| `data_model.py` | Typed dataclasses (432 lines): `EndpointInfo`, `ServiceInfo`, `VulnerabilityInfo`, `HostInfo`, `CredentialInfo` for normalizing data exchange between phases. `normalize_dkg_state()` / `to_prompt_context()`. |
...
| `darwin/tools/recon_server.py` | 9 recon tools: nmap_scan, whatweb_scan, dirb_scan, gobuster_dir, curl_get, http_post, form_extract, try_login, response_parse. |
| `darwin/tools/attack_server.py` | 67 tools across categories: SQL injection (sqlmap_test), fuzzing (ffuf_fuzz, send_payload, command_injection_test, xss_reflection_test, ssti_inject), brute force (hydra_http_brute, hydra_ssh_brute, wp_xmlrpc_brute), DB/NoSQL clients (mysql_query, mysql_file_write, psql_query, redis_cmd, mssql_query, mssqlclient_query, oracle_query, mongodb_query, elasticsearch_query, couchdb_query), AD/Windows (netexec_enum, netexec_ldap_enum, impacket_secretsdump, impacket_psexec, impacket_wmiexec, ldapsearch_ad, impacket_GetUserSPNs, impacket_GetNPUsers, impacket_secretsdump_dcsync, impacket_pth, impacket_ticketer, impacket_silver_ticket, impacket_ntlmrelayx, impacket_getST, smb_client, gpp_decrypt, hash_crack), K8s/cloud (kubectl_auth_check, kubectl_get_secrets, kubectl_get_pods, kubectl_run, kubectl_get_clusterrolebindings, kubectl_exec, check_capabilities, check_mounts, check_cloud_metadata, sa_token_read, etcdctl_get, kubelet_probe, docker_registry, helm, aws_cli), post-exploit (ssh_exec, shell_exec, ssh_key_exec, linux_priv_check, file_upload, php_filter_chain), knowledge (knowledge_search, cve_lookup, metasploit_search, go_exploitdb_search, searchsploit_search, searchsploit_copy, ddg_web_search), specialized exploits (tomcat_exploit, oracle_tns_poison, wpscan_enum, jwt_forge, test_credential, smbmap_enum, ssrf_probe, xxe_inject, graphql_introspect), and Linux privesc (SUID, capabilities, cron, LD_PRELOAD). Domain filtering via `_apply_domain_filter()` removes AD (27 tools), LNX (1 tool), or CLOUD_EXTRA (2 tools) based on `config/darwin.yaml` `tools.enabled_domains`. |
...
18. **Tool domain filtering**: `_apply_domain_filter()` in `attack_server.py` removes tools for disabled domains per `config/darwin.yaml` `tools.enabled_domains`. Domain categories: `_AD_TOOLS` (27), `_LNX_TOOLS` (1), `_CLOUD_EXTRA_TOOLS` (2). Default: no filtering (all 67 tools available).
19. **Exploit task prioritization**: `_EXPLOIT_PRIORITY` set ensures container escape, K8s exploitation, cloud, and post-exploit tasks run before low-confidence web probing tasks (XSS, SQLI, IDOR). Prevents high-value exploit chains from being starved by speculative recon tasks when time budget is tight.
20. **Chain mode auto-detection**: `_detect_chain_topology()` identifies multi-flag attack chains from service topology at bootstrap. When active, the orchestrator targets intermediate flags and continues to the next chain step rather than terminating after a single flag. Configurable via `config/darwin.yaml` (`chain_mode: auto`, `chain_max_flags: 10`).
...
- `/home/kianabin/benchmark_design/benchmarks/BENCHMARK_SUMMARY.md` — Comprehensive exploitation guide covering deployable scenarios and attack chains.
- `/home/kianabin/benchmark_design/benchmarks/BENCHMARK_SCENARIOS_OVERVIEW.md` — Concise overview table of scenarios and attack chains.

## Testing

### Current state
Only 6 of ~25+ modules have tests (146 total test methods), covering pure data-structure code:
...
## What is not yet integrated / outstanding
...
- Core orchestrator, all agents, tools, RAG, DPM, CTEG, DAVE, and LLM utilities are untested (only 6 modules — DKG, metrics, analysis, dynamic_scaling, MCP gateway, phase_logger — have test coverage via 146 tests)
- **Already integrated** (removed from this list): CTAGE (cloud_topology.py), CDF (dpm.py CloudDefenseProfile), CADS (dynamic_scaling.py continuous env_complexity), chain checkpoint & resume (chain_runner.py), multi-step exploit continuity (_extract_recent_artifacts), Linux privesc auto-exploitation (_execute_privesc), DB default credential auto-trial (_try_db_default_credentials), defense evasion context (_build_defense_evasion_context), analyze re-trigger on new discoveries
