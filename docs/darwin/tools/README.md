...
工具边界层。`mcp_gateway.py` 统一调用，`spec.py` 定义契约，`request_template.py` 定义 HTTP 请求形状与各工具的渲染规则，`params.py` 归一化参数形状，`attack_server.py`/`recon_server.py` 注册工具，`mcp_client.py` 连接可选外部 MCP，`adapters/` 负责能力到工具的参数适配。

## 推荐阅读顺序

`spec.py` → `mcp_gateway.py` → `request_template.py` → `params.py` → `recon_server.py` / `attack_server.py` → `manifest.py` → `adapters/`。
