...
`run()` 在 `next_ready()` 返回空、准备触发一次 stall replan 之前，会比较
`_world_signature()`（endpoints/services/credentials/vulnerabilities 计数）。
指纹没有变化说明这次重排没有任何新信息可依据，直接以 `plan_exhausted` 收敛，
不产生 "0 iterations, 1 replans" 这类空转（benchmark 日志里连续出现四次）。
