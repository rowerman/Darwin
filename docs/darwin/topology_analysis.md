...
- 服务暴露关系（`service_exposes_endpoint`/`endpoint_backed_by_service`）与 AWS 暴露关系（`resource_exposed_via`）会自动创建缺失的 Endpoint 节点，来源标记为 `relation_analyzer`。in-cluster Endpoint 只对带 `cluster_ip`/`k8s_namespace` 的 K8s Service 节点生成：端口取自节点自身的 `port`（旧的 `ports` 列表形态仍兼容），URL 形如 `http://<cluster_ip>:<port>`；主机/网络服务不生成 `cluster.local` 别名。
