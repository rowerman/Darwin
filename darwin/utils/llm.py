...
class LLMSession:
    ...
    def compress(
        ...
        if summarizable:
            ...
            # stage= is a thought-chain feature (generate() gained it in the
            # think-chain commit). main's generate() predates it, so pass it
            # conditionally to stay compatible with both codebases.
            _gen_kwargs = {
                "prompt": compression_prompt + serialized,
                "system_prompt": SYSTEM_PROMPT_COMPRESS,
            }
            try:
                if "stage" in inspect.signature(self.generate).parameters:
                    _gen_kwargs["stage"] = "compress"
            except Exception:
                pass
            try:
                summary, _ = self.generate(**_gen_kwargs)
            except Exception:
